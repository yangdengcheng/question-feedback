...
仓库根目录的 `CHANGELOG.md` 是唯一的发布记录，随代码一起走 git，规则如下：

1. **开发者**：完成功能/修复后，在 CHANGELOG 顶部 `[未发布]` 区块追加一条，与代码放在**同一个提交**里；没有 `[未发布]` 区块就新建一个。分类用 `新增 / 变更 / 修复 / 文档`，一条一行说清改了什么、对用户有什么影响。
2. **发布时**：push 前把 `## [未发布]` 改为发布日期（如 `## 2026-08-05`）。
3. **部署者**：`git pull` 会自动带上最新 CHANGELOG，无需额外操作。部署前先扫一眼本次条目：
   - 涉及**数据库结构变更** → 按条目说明执行 `npm run migrate` 或手工 SQL；
   - 纯代码变更 → 直接重启服务即可。
4. **记录标准**：新功能、权限变化、用户可感知的 Bug 修复、数据库结构变更（必记）、部署方式变化需要记录；错别字、不改行为的纯重构不用记。
5. **禁止**在服务器上直接修改 CHANGELOG 或任何文件，一切变更通过 git 提交体现。
